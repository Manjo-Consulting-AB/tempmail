...
/* CLIENT_AGENT_INSTALL_CONFIG_START */
...
$useExternalBootstrap = getenv('CLIENT_AGENT_USE_EXTERNAL_BOOTSTRAP') === '1';
if ($useExternalBootstrap && is_file($bootstrapPath)) {
    require_once $bootstrapPath;
}

if (!function_exists('clientAgentRunCycle')) {
    if (!defined('CLIENT_AGENT_BOOTSTRAPPED')) {
        define('CLIENT_AGENT_BOOTSTRAPPED', true);
        define('CLIENT_AGENT_ROOT', __DIR__);
    }

    function clientAgentGetEmbeddedConfig(): array {
        global $clientAgentEmbeddedConfig;
        return is_array($clientAgentEmbeddedConfig) ? $clientAgentEmbeddedConfig : [];
    }

    function clientAgentGetEmbeddedPrivateRoot(): string {
        $config = clientAgentGetEmbeddedConfig();
        $value = $config['private_root'] ?? '';
        return is_string($value) ? trim($value) : '';
    }

    function clientAgentGetEmbeddedDefaultScriptId(): string {
        $config = clientAgentGetEmbeddedConfig();
        $value = $config['default_script_id'] ?? '';
        return is_string($value) ? trim($value) : '';
    }

    function clientAgentGetPrivateRoot(): string {
        $embedded = clientAgentGetEmbeddedPrivateRoot();
    ...
    function clientAgentSaveLocalConfigMap(array $map): bool {
        $mapPath = clientAgentGetLocalConfigMapPath();
        ...
        if (file_put_contents($mapPath, $json . PHP_EOL) === false) {
            return false;
        }
    ...
    function clientAgentLoadEncryptedLocalImapSettings(?string $scriptId = null): array {
        ...
        if ($configPath === null) {
            return [
                'success' => false,
                'error' => 'Local IMAP config not found for script_id',
            ];
        }
...
if (defined('CLIENT_AGENT_LIBRARY_ONLY') && CLIENT_AGENT_LIBRARY_ONLY) {
    return;
}
...
if ($isCli) {
    ...
    if ($cmd === '--install' || $cmd === 'install') {
        fwrite(STDERR, "Installer has moved. Run: php install.php --install" . PHP_EOL);
        exit(2);
    }
}

$embeddedDefaultScriptId = function_exists('clientAgentGetEmbeddedDefaultScriptId') ? clientAgentGetEmbeddedDefaultScriptId() : '';
$scriptId = $_GET['script_id'] ?? ($_SERVER['argv'][1] ?? ($embeddedDefaultScriptId !== '' ? $embeddedDefaultScriptId : 'demo'));
$action = $_GET['action'] ?? ($_SERVER['argv'][2] ?? 'run');
$requestMethod = $_SERVER['REQUEST_METHOD'] ?? '';

if ($requestMethod === 'POST' || $action === 'update_lists') {
    $rawBody = file_get_contents('php://input');
    $payload = json_decode($rawBody, true);
    if (is_array($payload) && empty($payload['signature']) && !empty($_SERVER['HTTP_X_CLIENT_WEBHOOK_SIGNATURE'])) {
        $payload['signature'] = $_SERVER['HTTP_X_CLIENT_WEBHOOK_SIGNATURE'];
    }
    if (!is_array($payload) && !empty($_GET)) {
