...
Client Agent is built with privacy in mind, so everything runs on your server. If you want, you can download the agent, review the code to ensure it does what we say it does (it's in plain php), add the configuration files manually, and we would not know a thing about what happens there. It will just keep running quietly on your own box, doing its job.
