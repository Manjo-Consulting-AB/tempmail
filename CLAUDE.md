...
No `.env.example` is committed. Key env vars referenced in code (names only): `TEMPMAIL_ENV_FILE`, `ENV_FILE_PATH`, `TEMPMAIL_ENV_DIR`, `SECURE_ENV_DIR`, `DOCKER_ENV`, `APP_ENV`, `ENV`, `EMAIL_DOMAIN`, `BASE_URL`, `LOG_RETENTION_DAYS`, `CRON_HTTP_SECRET`, `SMTP_MAX_PER_RUN`, `SMTP_BATCH_SIZE`, `SMTP_PER_MINUTE`, `BMAC_WEBHOOK_SECRET`, `DB_HOST`, `DB_PORT`, `DB_SOCKET`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `IMAP_SERVER`, `IMAP_USER`, `IMAP_PASSWORD`, `IMAP_ENABLED`, `DEBUG_MODE`, `LOG_LEVEL`, `ATTACHMENT_TTL`, `ATTACHMENT_DOWNLOAD_SECRET`, `CLIENT_AGENT_PRIVATE_ROOT`, `TOTP_ENCRYPTION_KEY`.
